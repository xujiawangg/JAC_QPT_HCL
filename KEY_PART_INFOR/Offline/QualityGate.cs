...
namespace HfutIe.offline
{
    //1.正常下线；操作数据库
    //2.下线失败；在线销项/强制放行
    public partial class QualityGate : Form,IOPC
    {
        #region 0 基础变量
        #region 静态变量
        public static string station_key;
        public string wc_code;
        public string pro_line_name_C = ConfigurationManager.AppSettings.Get("pro_line_name_C");
        public string workcenter_code = ReadXML.GetWCCode(System.Windows.Forms.Application.StartupPath + @"\WCConfig\WC.xml");//工作中心编号
        string OpcName;
        ...
        private static string ServerIP = "10.4.5.102";//服务器的IP地址
        #endregion
        #endregion
        ...
        public QualityGate()
        {
            ...
            this.Height = 1024;
            wc_code = getwc_code();
            titleLib.Text = wc_code + "质量门";
        }
        #endregion

        #region  界面信息展示，自动流程运行
        private void QualityGate_Load(object sender, EventArgs e)
        {
            //服务器通讯timer启动
            ...
            while (true)
            {
                ...
                {
                    Log.GetInstance.WriteLog("监控地址已找到！");
                    break;
                }
            }
            ...
            {
                string IP = ConfigurationManager.AppSettings["KepServerIP"].ToString().Trim();
                OPCHelper<QualityGate>.OPC_Connect(this, IP, ControlAddressList);
            }
            ));
        ...
        private void ReFreshAddress()
        {
            try
            {
                wc = WORK_CENTERRepository.Repository().FindList().Where(s => s.WORK_CENTER_CODE == workcenter_code).ToList();//根据工作中心编号找到对应的工作中心
                if (wc.Count > 0)
                {
                ...
                {
                    Log.GetInstance.WriteLog("未找到该工作中心!");
                }
            }
            catch (Exception ex)
            {
        ...
        public void DataChange(ItemValueResult item)
        {
            ...
            {  //根据地址的类型，判断执行哪项逻辑操作
                ...
                string values = OPCHelper<QualityGate>.SynReadOpcItem(OpcName);//读取地址的value,即产品到位信号是否从0-1,即此时读到的信息为True时读取产品出生证信息
                if (values == "True")
                {
                    GetBornCode(OpcName);
                }
        ...
        public void GetBornCode(string opcname)
        {
            ...
            if (OpcItem != null)
            {
                ...
                {
                    int c = (int)AddressCategory.allet_arrive;//跳变信号类型
                    if (OpcItem.ADDRESS_TYPE == c.ToString())
                    {
                        ...
                        //if (is_leave_ok != null)
                        //{
                        //    OPCHelper<QualityGate>.SynWriteOpcItem(is_leave_ok.ADDRESS_PATH, "0");//复位放行信号
                        //}
                        //else
                        //{
                        //    Log.GetInstance.WriteLog("复位放行信号失败！");
                        //}
                        #endregion
                        ...
                        {
                            string values = OPCHelper<QualityGate>.SynReadOpcItem(pro_born_address.ADDRESS_PATH);//读取地址的value,即产品出生证信息
                            if (plan_product_data == null || (plan_product_data != null && plan_product_data.PRODUCT_BORN_CODE != values)) ;//如果读取的产品出生证与上一个产品出生证不一样时处理信息
        ...
        public void GetProductBornCode(string product_born_code)
        {
            ...
            try
            {
                ...
                if (isValid == true)
                {
                    ...
                    //    MyMsgBox.Show("当前产品已经下线，请重新输入！", "提醒", MyMsgBox.MyButtons.OK, MyMsgBox.MyIcon.Information,3);
                    //    return;
                    //}
                }
                else
                {
                    ...
                    SetVisible(arrowpicture1_pic, true);
                    MyMsgBox.Show("当前产品出生证无效！，请重新输入！", "提醒", MyMsgBox.MyButtons.OK, MyMsgBox.MyIcon.Information, 3);
                    return;
                }
            }
            catch (Exception ex)
            {
                MyMsgBox.Show(ex.Message, 3);
            }
        }
        #endregion
        #endregion
        ...
        private void confirm_btn_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else { MyMsgBox.Show("请输入产品出生证！", "提醒", MyMsgBox.MyButtons.OK, MyMsgBox.MyIcon.Information, 3); }
            }
            catch (Exception ex)
            {
                Log.GetInstance.WriteLog(ex.Message);
        ...
        private Boolean CheckValid(string code)
        {
            ...
            list = P_ASSEMBLE_PRODUCT_STATERepository.Repository().FindList($" and PRODUCT_BORN_CODE='{code}'");
            if (list.Count == 0)
                result = false;
            else
        ...
        {
            Boolean result = true;
            //string str = "select * from P_ASSEMBLE_PRODUCT_STATE where PRODUCT_BORN_CODE ='" + code + "'";
            //DataTable dt = DBhelperOracle.OpenTable(str);
            //P_ASSEMBLE_PRODUCT_STATE p = EntityHelper<P_ASSEMBLE_PRODUCT_STATE>.GetEntity(dt);
            List<P_ASSEMBLE_PRODUCT_STATE> list = new List<P_ASSEMBLE_PRODUCT_STATE>();
            list = P_ASSEMBLE_PRODUCT_STATERepository.Repository().FindList($" and PRODUCT_BORN_CODE='{code}'");
        ...
        {
            wc = WORK_CENTERRepository.Repository().FindList().Where(s => s.WORK_CENTER_CODE == workcenter_code).ToList();//根据ip找到对应的工作中心
            if (wc.Count > 0)
            {
                station = STATIONRepository.Repository().FindList().Where(s => s.WORK_CENTER_KEY == wc[0].WORK_CENTER_KEY).ToList();//根据工作中心找到对应的工位
                if (station.Count > 0)
                {
                    station_key = station[0].STATION_KEY;
                    return station[0].STATION_CODE;
                }
                else
                {
                    Log.GetInstance.WriteLog("未找到该工位，请在上层检查工位与工作中心配置关系!");
        ...
        private delegate void SetEnableDelegate(Button btn, bool st);
        private void SetEnable(Button txt, bool st)
        {
            if (txt.InvokeRequired) Invoke(new SetEnableDelegate(SetEnable), txt, st);
            else txt.Enabled = st;
        }
        /// <summary>
        /// 使用委托为Label赋值
        ...
        private void equipconnect_tmr_Tick(object sender, EventArgs e)
        {
            try
            {
                Showequipmentstate();
        ...
        public void ShowEquipConnection(bool state)
        {
            if (state)
            {
                //SetLblColors(serverconnectionstate_lbl, Color.ForestGreen);
        ...
        private delegate void ShowLineServerStateDelegate();
        private void Showequipmentstate()
        {
            //TimeSpan dt = recenttime - lasttime;
            //if (dt.Seconds >= 10)
            //{
            //    EquipCommunicationState = false;
            //    ShowEquipConnection(false);
            //}
            //else
            //{
            //    EquipCommunicationState = true;
            //    ShowEquipConnection(true);
            //}
            if (this.InvokeRequired)
            {
                ShowLineServerStateDelegate d = new ShowLineServerStateDelegate(Showequipmentstate);
                this.Invoke(d, new object[] { });
            }
            else
            {
                try
                {
                    ...
                    string result = OPCHelper<QualityGate>.SynReadOpcItem(pro_born_address.ADDRESS_PATH);
        ...
        private void Process()
        {
            ...
            try
            {
                list = P_ASSEMBLE_PRODUCT_STATERepository.Repository().FindList($" and PRODUCT_BORN_CODE='{productborncode_txt.Text}'");
                if (list.Count == 0)
                {
                    MyMsgBox.Show("当前产品出生证无关联信息，请重新输入！", "提醒", MyMsgBox.MyButtons.OK, MyMsgBox.MyIcon.Information, 3);
                    return;
                }
                else
                {
                    ...
                    if (planstate_lbl.BackColor == Color.FromArgb(57, 204, 36))
                    {
                        ...
                        if (is_leave_ok != null)
                        {
                            OPCHelper<QualityGate>.SynWriteOpcItem(is_leave_ok.ADDRESS_PATH, "1");//写入允许放行
                        }
                        else
                        {
                            SetLabel(offlinestate_lbl, false);
                    ...
                    {
                        DialogResult result = MyMsgBox.Show("当前产品系统检测不合格，以合格状态流转？", "自动流转", MyMsgBox.MyButtons.YesNo, 5);
        ...
        public void LoadDgv(DataGridView dgvSafeInfo, P_ASSEMBLE_PRODUCT_STATE paps, List<P_KEY_PART_C> lkpc, List<P_KEY_PART_INFOR> lpkpi)
        {
            ...
            else
            {
                for (int row = 0; row < lkpc.Count; row++)
                {
                    ...
                    int difference = (int)lkpc[row].QUANTITY - pkpil.Count;
            ...
            else
            {
                for (int row = 0; row < lpc.Count; row++)
                {
                    ...
                    if (tempdt != null && tempdt.Count > 0)
                    {
                        ...
                        dgvQuaInfo.Rows[row].Cells["IS_CLT"].Value = tempdt[0].CHECK_VALUE == null ? "无" : "是";
                        if (tempdt[0].IS_QUALIFIED == 1|| tempdt[0].IS_QUALIFIED == 4)
                        {
        ...
        #region 10 显示拧紧数据
        private delegate void LoadDgvDelegatePtc(DataGridView dgv, P_ASSEMBLE_PRODUCT_STATE paps, List<PRODUCT_DCS_PARM_LIST> lpc, List<P_TIGHTENING> lptc);
        public void LoadDgv(DataGridView dgvTightInfo, P_ASSEMBLE_PRODUCT_STATE paps, List<PRODUCT_DCS_PARM_LIST> lpc,List<P_TIGHTENING> lptc)
        {
            List<DCS> dcs = new List<DCS>();
            List<DCS_PARM> dcs_p = new List<DCS_PARM>();
            if (dgvTightInfo.InvokeRequired)
            {
                LoadDgvDelegatePtc dd = new LoadDgvDelegatePtc(LoadDgv);
                dgvTightInfo.Invoke(dd, new object[] { dgvTightInfo, paps, lpc,lptc });
            }
            else
            {
                for (int row = 0; row < lpc.Count; row++)
                {
                    ...
                    dgvTightInfo.Rows[row].Cells["STATION_KEY"].Value = lpc[row].STATION_KEY;
                    dcs = DCSRepository.Repository().FindList().Where(s => s.dcs_key == lpc[row].DCS_KEY).ToList();
                    dgvTightInfo.Rows[row].Cells["STATION_CODE"].Value = dcs[0].dcs_code;
                    dgvTightInfo.Rows[row].Cells["STATION_NAME"].Value = dcs[0].dcs_name;
                    DCS_PARM dcsparminfor = DCS_PARMRepository.Repository().FindEntityBySql($"select * from dcs_parm where DCS_KEY = '" + lpc[row].DCS_KEY + "' and PARM_KEY ='" + lpc[row].PARM_KEY+"'");
                    var tempdt= lptc.Where(a => a.STATION_KEY == lpc[row].STATION_KEY && a.Parm_Name == dcsparminfor.PARM_NAME).ToList();
                    if(tempdt!=null && tempdt.Count>0)
                    {
                        dgvTightInfo.Rows[row].Cells["UPPER_CONTROL_TORQUE"].Value = tempdt[0].upper_control_torque;
                        dgvTightInfo.Rows[row].Cells["LOWER_CONTROL_TORQUE"].Value = tempdt[0].lower_control_torque;
                        dgvTightInfo.Rows[row].Cells["CHECK_VALUE_TORQUE"].Value = tempdt[0].check_value_torque;
                        dgvTightInfo.Rows[row].Cells["UPPER_CONTROL_ANGLE"].Value = tempdt[0].upper_control_angle;
                        dgvTightInfo.Rows[row].Cells["LOWER_CONTROL_ANGLE"].Value = tempdt[0].lower_value_angle;
                        dgvTightInfo.Rows[row].Cells["CHECK_VALUE_ANGLE"].Value = tempdt[0].check_value_angle;
                        dgvTightInfo.Rows[row].Cells["CHECK_TIME"].Value = tempdt[0].Check_Time;
                        dgvTightInfo.Rows[row].Cells["IS_QUALIFIED"].Value = tempdt[0].is_qualified == "1" || lptc[row].is_qualified == "4" ? "合格" : "不合格";
                    }
                    else
                    {
                        dgvTightInfo.Rows[row].Cells["CHECK_VALUE_ANGLE"].Value = "无";
                        dgvTightInfo.Rows[row].Cells["CHECK_TIME"].Value = "无";
                        dgvTightInfo.Rows[row].Cells["IS_QUALIFIED"].Value = "不合格";
                    }
                    if (dgvTightInfo.Rows[row].Cells["IS_QUALIFIED"].Value.ToString() == "不合格")
                    {
            ...
            for (int row = 0; row < dgvTightInfo.Rows.Count; row++)
            {
        ...
        private void LoadQuaInfo(P_ASSEMBLE_PRODUCT_STATE paps)
        {
            ...
            List<DCS_PARM> dsc_parm = new List<DCS_PARM>();
            try
            {
                List<DCS> dcs = DCSRepository.Repository().FindList();
                List<DCS> dcsinfor = new List<DCS>();
                dsc_parm = DCS_PARMRepository.Repository().FindListBySql($"select * from dcs_parm where PARM_TYPE like '%质量数采%'");
                QG = QUALITY_GATERepository.Repository().FindList($" and CFG_STATION_KEY='{station_key}'");
                STA = STATIONRepository.Repository().FindList().Where(s => QG.Select(q => q.STATION_KEY).Contains(s.STATION_KEY)).ToList();
                dcsinfor = dcs.Where(s => STA.Select(q => q.STATION_CODE).Contains(s.dcs_code)).ToList();
                lpc = PRODUCT_DCS_PARM_LISTRepository.Repository().FindList($" and PART_KEY='{paps.PRODUCT_KEY}'").Where(s => dcsinfor.Select(q => q.dcs_key).Contains(s.DCS_KEY) && dsc_parm.Select(d=>d.PARM_KEY).Contains(s.PARM_KEY)).ToList();//上层新建DCS是否是质量门已弃用
                if (lpc.Count > 0)
                {
                    for (int row = 0; row < lpc.Count; row++)
                    {
                        DCS dcs_sta = dcs.FirstOrDefault(s => s.dcs_key == lpc[row].DCS_KEY);
                        string str3 = "select * from P_DCS_" + dcs_sta.dcs_name.ToString() + " where DCS_KEY ='" + lpc[row].DCS_KEY + "' and PARM_KEY='" + lpc[row].PARM_KEY + "'and PRODUCT_BORN_CODE='" + paps.PRODUCT_BORN_CODE + "'";
                ...
                {
                    MyMsgBox.Show("该产品质量检验信息未配置", "提醒", MyMsgBox.MyButtons.OK, MyMsgBox.MyIcon.Information, 3);
                    SetLabel(lblQua, false);
                    return;
                }
            }
        ...
        private void LoadSafeInfo(P_ASSEMBLE_PRODUCT_STATE paps)
        {
            try
            {
                ...
                QG = QUALITY_GATERepository.Repository().FindList($" and CFG_STATION_KEY='{station_key}'");
                //STA = STATIONRepository.Repository().FindList().Where(s => QG.Select(q => q.STATION_KEY).Contains(s.STATION_KEY)).ToList();
                lkpc = P_KEY_PART_CRepository.Repository().FindList($" and PRODUCT_KEY='{paps.PRODUCT_KEY}' and MES_PLAN_KEY='{paps.MES_PLAN_KEY}'").Where(s =>QG.Select(q => q.STATION_KEY).Contains(s.STATION_KEY)).ToList();
                if (lkpc.Count > 0)
                {
                    lpkpi = P_KEY_PART_INFORRepository.Repository().FindList($" and PRODUCT_BORN_CODE='{paps.PRODUCT_BORN_CODE}'").Where(s => lkpc.Select(q => q.STATION_KEY).Contains(s.STATION_KEY)).ToList();
                    LoadDgv(dgvSafeInfo, paps, lkpc, lpkpi);
                }
                else
                {
                    MyMsgBox.Show("该产品安全件信息未配置", "提醒", MyMsgBox.MyButtons.OK, MyMsgBox.MyIcon.Information, 3);
                    SetLabel(lblSafe, false);
                    return;
                }
            }
        ...
        private void LoadTightInfo(P_ASSEMBLE_PRODUCT_STATE paps)
        {
            ...
            try
            {
                SetLabel(lblTight, true);
                List<DCS> dcsinfor = new List<DCS>();
                //dsc_parm = DCS_PARMRepository.Repository().FindListBySql($"select * from dcs_parm where PARM_TYPE like '%拧紧数采%'");
                QG = QUALITY_GATERepository.Repository().FindList().Where(s => s.CFG_STATION_KEY == station_key).ToList();
                STA = STATIONRepository.Repository().FindList().Where(s => QG.Select(q => q.STATION_KEY).Contains(s.STATION_KEY)).ToList();
                dcsinfor = DCSRepository.Repository().FindList().Where(s => STA.Select(q => q.STATION_CODE).Contains(s.dcs_code)).ToList();
                //lpc = PRODUCT_DCS_PARM_LISTRepository.Repository().FindList($" and PART_KEY='{paps.PRODUCT_KEY}'").Where(s =>dcsinfor.Select(q => q.dcs_key).Contains(s.DCS_KEY) && dsc_parm.Select(d => d.PARM_KEY).Contains(s.PARM_KEY)).ToList();
                lpc = PRODUCT_DCS_PARM_LISTRepository.Repository().FindList($" and PART_KEY='{paps.PRODUCT_KEY}'");
                //lptc = P_TIGHTENINGRepository.Repository().FindList().Where(s => s.PRODUCT_KEY == paps.PRODUCT_KEY && s.PRODUCT_BORN_CODE == paps.PRODUCT_BORN_CODE && QG.Select(q => q.STATION_KEY).Contains(s.STATION_KEY) && dsc_parm.Select(d => d.PARM_KEY).Contains(s.PARM_KEY)).ToList();
                if (lpc.Count > 0)
                {
                    for(int row=0;row< lpc.Count;row++)
                    {
                        DCS_PARM dcsparminfor = DCS_PARMRepository.Repository().FindEntityBySql($"select * from dcs_parm where DCS_KEY = '" + lpc[row].DCS_KEY + "' and PARM_KEY ='" + lpc[row].PARM_KEY+"'");
                        //P_TIGHTENING pt = P_TIGHTENINGRepository.Repository().FindEntityBySql($"select * from P_TIGHTENING where PRODUCT_KEY='" + paps.PRODUCT_KEY + "' and PRODUCT_BORN_CODE ='" + paps.PRODUCT_BORN_CODE + "' and STATION_KEY ='" + lpc[row].STATION_KEY + "' and PARM_NAME ='" + dcsparminfor.PARM_NAME + "'");
                        P_TIGHTENING pt = P_TIGHTENINGRepository.Repository().FindEntityBySql($"select * from P_TIGHTENING where PRODUCT_KEY='" + paps.PRODUCT_KEY + "' and PRODUCT_BORN_CODE ='" + paps.PRODUCT_BORN_CODE + "' and STATION_KEY ='" + lpc[row].STATION_KEY + "");
                        if (pt != null && pt.tightening_key != null)
                        { lptc.Add(pt); }
                    }
                    //lptc = P_TIGHTENINGRepository.Repository().FindList().Where(s => s.PRODUCT_KEY == paps.PRODUCT_KEY && s.PRODUCT_BORN_CODE == paps.PRODUCT_BORN_CODE && QG.Select(q => q.STATION_KEY).Contains(s.STATION_KEY)).ToList();
                    LoadDgv(dgvTightInfo, paps, lpc, lptc);
                }
                else
                {
                    MyMsgBox.Show("该产品拧紧质量数据信息未配置", "提醒", MyMsgBox.MyButtons.OK, MyMsgBox.MyIcon.Information, 3);
                    SetLabel(lblTight, false);
                    return;
                }
            }
        ...
        private void online_delete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (paps == null)
                    {
                        MyMsgBox.Show("系统无该出生证相关记录！", "信息提示", MyMsgBox.MyButtons.OK, MyMsgBox.MyIcon.Information, 3);
                        return;
                    }
                    else
                    {
                        ...
                        {
                            MyMsgBox.Show("未选中销项数据！", "提示信息", MyMsgBox.MyButtons.OK, MyMsgBox.MyIcon.Error, 3);
                            return;
                        }
                        else if (dgvQuaInfo.SelectedRows.Count != 0)
                        {
                            DialogResult result = MyMsgBox.Show("是否要销项当前行中单元格的内容?", "提示信息", MyMsgBox.MyButtons.YesNo, MyMsgBox.MyIcon.Question, 3);
                            if (result == DialogResult.Yes)
                            {
                                if (dgvQuaInfo.CurrentRow.Cells["CLT_RESULT"].Value.ToString() == "合格")
                                {
                                    MyMsgBox.Show("该项数据合格！", "提示信息", MyMsgBox.MyButtons.OK, MyMsgBox.MyIcon.Error, 3);
                                ...
                                if (qe.Count > 0)
                                {
                                    MyMsgBox.Show("该项数据已销项！", "提示信息", MyMsgBox.MyButtons.OK, MyMsgBox.MyIcon.Error, 3);
                                    dgvQuaInfo.ClearSelection();
                                    return;
                                }
                                else
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (result == true)
                    {
                        SetLabel(barcodestate_lbl, true);
                        SetVisible(arrowpicture1_pic, true);
                        DialogResult release = MyMsgBox.Show("是否将该产品以不合格状态下线?", "提示信息", MyMsgBox.MyButtons.YesNo, MyMsgBox.MyIcon.Question, 3);
                        if (release == DialogResult.Yes)
                        {
                            //UpdatePlanInfo(paps, 1);
                            InsertReleaseInfor(paps, 1);
                            SetLabel(offlinestate_lbl, true);
                            MyMsgBox.Show("该产品已强制放行！", "提醒", MyMsgBox.MyButtons.OK, MyMsgBox.MyIcon.Information, 3);
                            #region 对PLC写入允许放行信号
                            CONTROL_ADDRESS is_leave_ok = ControlAddressList.FirstOrDefault(s => s.ADDRESS_TYPE == ((int)AddressCategory.is_leaveOK).ToString());
                            if (is_leave_ok != null)
                            {
                            ...
                            { Log.GetInstance.WriteLog("当前工位无放行地址！"); }
                            #endregion
                        }
                        else
                        {
        ...
        private void InsertReleaseInfor(P_ASSEMBLE_PRODUCT_STATE paps, int i)
        {
            ...
            try
            {
                ...
                qgfr.STATION_KEY = station_key;//记录质量门的工位信息
