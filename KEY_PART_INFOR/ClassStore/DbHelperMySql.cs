...
namespace HfutIe
{
    public abstract class DbHelperMySql
    {
        ...
        //实验室DCT数据库连接（本机）
        public static string mysql_server_name_HCL = "localhost";
        public static string mysql_database_name_HCL = "jac_qpt_hcl";
        public static string mysql_username_HCL = "root";
        public static string mysql_password_HCL = "root";
        ...
        /// 将数据库连接参数拼接为连接语句
        /// </summary>
        private static void combineConString()
        {
            connectionString = "server = " + mysql_server_name_HCL + ";database = " + mysql_database_name_HCL + ";user id = " + mysql_username_HCL + ";password=" + mysql_password_HCL + "";
        }
        #region 公用方法
        ...
        /// <returns></returns>
        public static DataTable OpenTable(string MySQLString)
        {
            combineConString();
            DataTable dt = new DataTable();
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                MySqlDataAdapter da = new MySqlDataAdapter(MySQLString, connection);
                try
                {
                    da.Fill(dt);
                }
                catch (Exception ex)
                {
                    ...
                    // string errormessage = SQLString + "：" + ex.Message;
        ...
        public static int ExecuteMySql(string MySQLString)//执行SQL语句，返回影响的记录数
        {
            combineConString();
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
        ...
        public static string GetStrValue(string TableName, string ColumnName, string Value, string GoalColumn)
        {
            ...
            DataTable dt = data.DBQuery.OpenTable1(SqlStr);
            if (dt.Rows.Count > 0)
            {
        ...
        /// <param name="goalcolumn">目标列</param>
        /// <returns></returns>
        public static string[] GetStrValues(string TableName, string ColumnName, string Value, string GoalColumn)
        {
            string[] ReturnStr;
            string SqlStr = "select " + GoalColumn + " from " + TableName + " where " + ColumnName + " = '" + Value + "'";
            //DataTable dt = data.DBQuery.OpenTable1("select " + GoalColumn + " from " + TableName + " where " + ColumnName + " = '" + Value + "'");
            DataTable dt = data.DBQuery.OpenTable1(SqlStr);
            ReturnStr = new string[dt.Rows.Count];
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ReturnStr[i] = dt.Rows[i][0].ToString();
            }
            return ReturnStr;
        }
